...
class Block:
    def __init__(self, block_id):
        ...
        self.token_ids = []     # store the token ids in the block

    # update the block with new token ids (with its hash) --> assign function
    def update(self, hash: int, token_ids: list[int]):
        self.hash = hash
        self.token_ids = token_ids

    # clean the block --> reset function
    def reset(self):
        self.ref_count = 1
        self.hash = -1
        self.token_ids = []
...
class BlockManager:
    def __init__(self, num_blocks: int, block_size: int):
        assert num_blocks > 0
        self.block_size = block_size                                                # the size of each block to store tokens
    ...
    # only compute hash when the block is full
    @classmethod
    ...
    # allocate the blocks for the sequence, update the block table and hash table --> allocate only once for each seq
    def allocate(self, seq: Sequence):
        ...
        for i in range(seq.num_blocks):
            ...
            if cache_miss:
                ...
                seq.num_cached_tokens += self.block_size    # should add this?
                if block_id in self.used_block_ids:         # should add this?
                    block = self.blocks[block_id]
                    block.ref_count += 1
                else:
                    block = self._allocate_block(block_id)
    ...
    def may_append(self, seq: Sequence):
        ...
        if len(seq) % self.block_size == 1: # current the last block is full, the new added token will be in a new block
            ...
            h = self.compute_hash(token_ids, prefix) # calculate the hash of the last block (based on the previous block's hash)

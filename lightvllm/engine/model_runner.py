...
from lightvllm.utils.context import set_context, get_context, reset_context
...
class ModelRunner:
    ...
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        if self.world_size > 1:
            if rank == 0:
                self.shm = SharedMemory(name="lightvllm", create=True, size=2**20)
            else:
                pass  # other ranks waits untill rank 0 finishes creating shared memory
            dist.barrier()
            if rank == 0:
                pass
            else:
